...
child_loss <- function(countries, reference_years, ages_keep = 15:100, max_child_age = NA, path = "../../Data/derived", ASFRC,LTCB) {
  ...
  df_l <- lapply(
    ...
    , ages_keep = ages_keep
    , ASFRC
    , lt_df = LTCB %>% select(Country, Age, Cohort, lx)
...
# expected_child_death <- function(ASFRSC, lx.kids.arr, xs, mas, cos, ages_keep = NA, max_child_age = NA) {
#   # browser()
#   # Child loss matrix
...
offspring_death_prevalence <- function(k_value, file_name, years = 2010:2019, breaks = c(20, 45, 50), reprod_age = c(15,50), abs_df_all, ASFRC, LTCF, method = "mid-interval"){
  ...
  if(method == "mean"){
    output <-
      ...
      summarise(
        # Option 1, take mean
        bereaved_women = mean(bereaved_women, na.rm = T)
        , bereaved_mothers = mean(bereaved_mothers, na.rm = T)
      ) %>%
      ungroup %>%
    ...
    output <-
      ...
      summarise(
        # Option 1, take mean
        bereaved_women = median(bereaved_women, na.rm = T)
        , bereaved_mothers = median(bereaved_mothers, na.rm = T)
      ) %>%
      ungroup %>%
      mutate(iso = plyr::mapvalues(country, from =  old, to = new)) %>%
      select(iso, everything())
...
# Taken from DemoTools
qx2lx <- function (nqx, radix = 100000) {
...
survival_probs_over_age <- function(max_child_age, lx_indices, co, ma, lt_df) {
  ...
  upper <- co + ma - (max_child_age + 1)
  lower <- upper - sum(as.numeric(names(lx_indices)) > max_child_age) + 1
  y <- lower:upper
  
  lt_df %>% 
    # filter(Age == max_child_age) %>% 
    filter(Cohort %in% y) %>% 
    pull(lx)
...
worker_child_loss <- function(country_keep, reference_years, sex_keep = F, ages_keep, ASFRC,lt_df, max_child_age = NA, path) {
  ...
  # 2.3. Keep relevant colums of life table
  
  if(!is.na(max_child_age)) {
    lt_df <- lt_df %>%
      filter(Country %in% country_keep) %>% 
      filter(Age == max_child_age) %>% 
      select(Cohort, lx)  
...
# New code for appendix -----

compare_measures_bulk_5y <- function(measure, export, regions,  surv_df, name_export = ""){
  # browser()
  ...
  temp <- 
    lapply(seq_along(surv_measure_keep_all_n), function(n) {
      ...
      compare_measures_5y(
        year_for_missing_countries
        , surv_measure_keep = surv_measure_keep_all_n[n]
        , model_agegr_keep = model_agegr_keep_all_n[n]
        , measure
        , model_df = indicator_df
        , surv_df = surv_df
        , regions = regions
      ) %>% 
        mutate(
          measure = measure_temp
          , ages = ages_pretty_n[n]
          # , age_low = ages_low_n[n]
        )
    }) %>% 
    bind_rows()
  ...
  prevalence <- 
    temp %>% 
    mutate(
      level = paste0(measure, ages, "kc")
      , level = factor(level, levels = sort(unique(level)))
    ) %>%
    arrange(level) %>% 
    select(iso, level, model) %>% 
    pivot_wider(names_from = level, values_from = model) %>% 
    left_join(
      regions %>% 
        # filter(!grepl('COUNTRIES EXCLUDED', region)) %>% 
        select(iso, country)
