countries_final <- unique(cd$country)
...
fall_in_ci <- 
  om_comp %>% 
  mutate(ok = kc >= ci_low & kc <= ci_high)


sum(fall_in_ci$ok)/ nrow(fall_in_ci)

# Interprtation - only 6% of the KC estimates (measures) fall 
# within 95% CI of survey estimates.
# This is now what we expect...
...
out_es <- 
  ...
  select(country, iso, survey_year = year, variable, kc, everything()) %>% 
  mutate(ok95 = kc >= ci_low & kc <= ci_high) %>% 
  rename(surv_mean = mean, surv_lb95 = ci_low, surv_ub95 = ci_high, kc_mean = kc)
  # pivot_wider(names_from = variable, values_from = value) %>% 
  # arrange(source, country) %>% 
  # select(source, country, iso, survey_year = year, everything()) 

write.csv(out_es, "../../Output/_ggs_estimates_ci.csv", row.names = F)
