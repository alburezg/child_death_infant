...
# 2. Mother qx values from LTC

cd_mothers_qx <- merge(
  cd_mothers
  , LTCF %>% 
    select(country = Country, cohort = Cohort, age = Age, qx_mother_death = qx)
    # Note that the UN cohort life tables were
    # created with radiox = 1E6
    # so, to make this comparable to the values of OD
    # they must refer to a life table with radix = 1
    # mutate(qx = qx * 1E6)
  , by = c("country", "cohort", "age")
)
...
cd_table_mult <- 
  ...
  mutate(
    ...
    # , lx_women_surviving = lx / first(lx)
    # , lx_women_experienced_child_death = 1 - qx2lx(nqx = diff, radix = 1)
    # , bereaved_women_old =  lx_women_experienced_child_death * lx_women_surviving * 1000
    ...
    # , bereaved_mothers_old =  bereaved_women_old * share_of_women_are_mothers
  ) %>% 
  ungroup
